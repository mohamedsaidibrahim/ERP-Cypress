...
const reportFilePath = path.join(__dirname,"Output", 'v1_axe-unified-report.md');

// Function to run axe-cli for a given URL
function runAxeCli(url) {
    try {
        // Run axe-cli and capture the output
        const axeReport = execSync(`axe ${url} --quiet --json`, { encoding: 'utf-8' });
        const parsedReport = JSON.parse(axeReport);

        return parsedReport.violations.length > 0 ? parsedReport.violations : 'No violations found';
    } catch (error) {
